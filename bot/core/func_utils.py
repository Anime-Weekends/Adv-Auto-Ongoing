    ...
    try:
        for i, chat_id in enumerate(Var.FSUB_CHATS, 1):
            try:
                ...
                try:
                    member = await bot.get_chat_member(chat_id, uid)
                    status = "<blockquote>Jᴏɪɴᴇᴅ ✅️</blockquote>" if member.status not in ["left", "kicked"] else "<blockquote>Nᴏᴛ Jᴏɪɴᴇᴅ ❌</blockquote>"
                except UserNotParticipant:
                ...
                if status != "<blockquote>Jᴏɪɴᴇᴅ ✅️</blockquote>":
        ...
        if args:
            callback_data = f"start_{args[0]}" if isinstance(args, list) and len(args) > 0 else "start"
            btns.append([InlineKeyboardButton("Tʀʏ Aɢᴀɪɴ", callback_data=callback_data)])
        else:
            btns.append([InlineKeyboardButton("Tʀʏ Aɢᴀɪɴ", callback_data="start")])
